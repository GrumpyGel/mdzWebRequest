...
HttpPrivateGet          Function to make a http get to the Private Server and return the response as a string.
...
UrlPrivateServer        Used to locate secure information, for example database connection strings. Used by
                        the HttpPrivateGet function.
*/

  private DateTime    c_DateNull              = new DateTime(1, 1, 1, 0, 0, 0);
  private DateTime    c_DateDefault           = new DateTime(2020, 1, 1, 0, 0, 0);
  private string      c_UrlPrivateServer      = "";
...
*/
  ...
  private string      mdzSys_ErrorLog_From                = "";
  private string      mdzSys_ErrorLog_To                  = "";
...
*/
  ...
  private mdzSys()
  {
    ...
    try {
      i_Filename            = System.Web.HttpContext.Current.Server.MapPath("mdzSys.config");
      ...
      try { if (i_Root.HasAttribute("DateNull"))                    { c_DateNull                          = DateTime.Parse(i_Root.GetAttribute("DateNull"));      } } catch { i_Errors = i_Errors + ", DateNull"; }
      try { if (i_Root.HasAttribute("DateDefault"))                 { c_DateDefault                       = DateTime.Parse(i_Root.GetAttribute("DateDefault"));   } } catch { i_Errors = i_Errors + ", DateDefault"; }
      try { if (i_Root.HasAttribute("UrlPrivateServer"))            { c_UrlPrivateServer                  = i_Root.GetAttribute("UrlPrivateServer");              } } catch { i_Errors = i_Errors + ", UrlPrivateServer"; }

      i_Pos                 = "Smtp";
      i_Element             = i_Root.GetElementsByTagName("Smtp");
      if (i_Element.Count == 1) {
        i_Section           = (XmlElement) i_Element[0];
        try { if (i_Section.HasAttribute("Server"))                 { mdzSys_Smtp_Server                  = i_Section.GetAttribute("Server");                     } } catch { i_Errors = i_Errors + ", Smtp@Server"; }
        try { if (i_Section.HasAttribute("Authenticate"))           { mdzSys_Smtp_Authenticate            = ToBool(i_Section.GetAttribute("Authenticate"));       } } catch { i_Errors = i_Errors + ", Smtp@Authenticate"; }
        try { if (i_Section.HasAttribute("Authenticate_Port"))      { mdzSys_Smtp_Authenticate_Port       = i_Section.GetAttribute("Authenticate_Port");          } } catch { i_Errors = i_Errors + ", Smtp@Authenticate_Port"; }
        try { if (i_Section.HasAttribute("Authenticate_UserName"))  { mdzSys_Smtp_Authenticate_UserName   = i_Section.GetAttribute("Authenticate_UserName");      } } catch { i_Errors = i_Errors + ", Smtp@Authenticate_UserName"; }
        try { if (i_Section.HasAttribute("Authenticate_Password"))  { mdzSys_Smtp_Authenticate_Password   = i_Section.GetAttribute("Authenticate_Password");      } } catch { i_Errors = i_Errors + ", Smtp@Authenticate_Password"; }
        try { if (i_Section.HasAttribute("LogErrors"))              { mdzSys_Smtp_LogErrors               = ToBool(i_Section.GetAttribute("LogErrors"));          } } catch { i_Errors = i_Errors + ", Smtp@LogErrors"; }
        try { if (i_Section.HasAttribute("ThrowErrors"))            { mdzSys_Smtp_ThrowErrors             = ToBool(i_Section.GetAttribute("ThrowErrors"));        } } catch { i_Errors = i_Errors + ", Smtp@ThrowErrors"; } }

      i_Pos                 = "WriteLog";
      i_Element             = i_Root.GetElementsByTagName("WriteLog");
      if (i_Element.Count == 1) {
        i_Section           = (XmlElement) i_Element[0];
        try {  if (i_Section.HasAttribute("File"))                  { mdzSys_WriteLog_File                = i_Section.GetAttribute("File");                       } } catch { i_Errors = i_Errors + ", WriteLog@File"; } }

      i_Pos                 = "ErrorLog";
      i_Element             = i_Root.GetElementsByTagName("ErrorLog");
      if (i_Element.Count == 1) {
        i_Section           = (XmlElement) i_Element[0];
        try { if (i_Section.HasAttribute("File"))                   { mdzSys_ErrorLog_File                = i_Section.GetAttribute("File");                       } } catch { i_Errors = i_Errors + ", ErrorLog@File"; }
        try { if (i_Section.HasAttribute("From"))                   { mdzSys_ErrorLog_From                = i_Section.GetAttribute("From");                       } } catch { i_Errors = i_Errors + ", ErrorLog@From"; }
        try { if (i_Section.HasAttribute("To"))                     { mdzSys_ErrorLog_To                  = i_Section.GetAttribute("To");                         } } catch { i_Errors = i_Errors + ", ErrorLog@To"; }
        try { if (i_Section.HasAttribute("CC"))                     { mdzSys_ErrorLog_CC                  = i_Section.GetAttribute("CC");                         } } catch { i_Errors = i_Errors + ", ErrorLog@CC"; }
        try { if (i_Section.HasAttribute("ThrowErrors"))            { mdzSys_ErrorLog_ThrowErrors         = ToBool(i_Section.GetAttribute("ThrowErrors"));        } } catch { i_Errors = i_Errors + ", ErrorLog@ThrowErrors"; } }

      i_Pos                 = "PerformanceCheck";
      i_Element             = i_Root.GetElementsByTagName("PerformanceCheck");
      if (i_Element.Count == 1) {
        i_Section           = (XmlElement) i_Element[0];
        try { if (i_Section.HasAttribute("Enable"))                 { mdzSys_PerformanceCheck_Enable      = ToBool(i_Section.GetAttribute("Enable"));             } } catch { i_Errors = i_Errors + ", PerformanceCheck@Enable"; }
        try { if (i_Section.HasAttribute("File"))                   { mdzSys_PerformanceCheck_File        = i_Section.GetAttribute("File");                       } } catch { i_Errors = i_Errors + ", PerformanceCheck@File"; } }
      if (i_Errors != "") {
        ErrorLog("mdzSys.cs", "mdzSys()", "", "Errors were encountered setting the following properties : " + i_Errors.Substring(2)); } }
    catch(Exception ex) {
        ErrorLog("mdzSys.cs", "mdzSys()", "Initialisation at " + i_Pos, ex); }
  }
  ...
  public XmlElement ErrorLog(string p_Program, string p_Routine, string p_Action, string p_Source, string p_Message)
  {
    ...
    try {
      if (mdzSys_ErrorLog_File != "") {
        WriteLog(mdzSys_ErrorLog_File, "Prg:" + p_Program + ", Rtn:" + p_Routine + ", Act:" + p_Action + ", Src:" + p_Source + ", Msg:" + p_Message); } }
    catch {
...
/*
HttpPrivateGet Function
=======================

Makes a GET request to the private URL and returns the response.

Uses a standard httpWebRequest call as the mdzWebRequest is restricted from accessing the private server.
*/


  public string HttpPrivateGet(string p_URL)
  {
    WebRequest      i_Request;
    HttpWebResponse i_Response;
    Stream          i_ResponseStream;
    StreamReader    i_Reader;
    string          i_Content;

    try
    {
      i_Request             = WebRequest.Create(c_UrlPrivateServer + "/" +p_URL);
      i_Request.Method      = "GET";
      i_Response            = (HttpWebResponse) i_Request.GetResponse();
      if (i_Response.StatusCode != HttpStatusCode.OK) {
        i_Response.Close();
        throw new Exception("HTTP Status Code = " + i_Response.StatusCode); }
      i_ResponseStream      = i_Response.GetResponseStream();
      i_Reader              = new StreamReader (i_ResponseStream);
      i_Content             = i_Reader.ReadToEnd();
      i_Reader.Close();
      i_ResponseStream.Close();
      i_Response.Close(); }
    catch(Exception ex) {
      ErrorLog("mdzSys.cs", "HttpPrivateGet('" + p_URL + "')", "", ex);
      throw; }
    return i_Content;
  }
